...
class CompleteGoalList extends Component {
  ...
  sort(a, b) {
    ...
    if (index === 1) {
      ...
      return (this.state.orderAsc ? a.email.localeCompare(b.email) : b.email.localeCompare(a.email));
    } else if (index === 3) {
      return (this.state.orderAsc ? a.created.localeCompare(b.created) : b.created.localeCompare(a.created));
    } else if (index === 4) {
      return (this.state.orderAsc ? a.finished.localeCompare(b.finished) : b.finished.localeCompare(a.finished));
    } else if (index === 5) {
  ...
  searchType2(event) {
    this.setState({searchBy: 'email', searchPlaceholder: 'Введите email автора'});
  }
  ...
  render() {
    const headers = ["","Название","Создал(а)","Дата создания", "Дата завершения","Статус","Категория"];
    ...
    if (searchBy === 'title') {
      ...
      if (searchString.length > 0) {
        goalslist = goalslist.filter((goal) => {
          return goal.email.toLowerCase().match( searchString );
        });
      }
              ...
              return (
                <div>
                  <div className="row" style={{marginBottom: '8px'}}>
                    <div className="col-lg-3 col-sm-4 caption">
                      <h4 style={{color: '#FFFFFF'}}>Завершенные задачи</h4>
                    </div>
                    <div className="col-lg-4 col-sm-8 offset-lg-2 buttons">
